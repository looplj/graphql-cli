...
// FindScope controls which schema elements to search.
...
// IsEmpty returns true if no scope is set (meaning search all).
...
// Find searches the given schema for definitions matching keyword and scope.
func Find(s *Schema, keyword string, scope FindScope) ([]FindResult, error) {
	return FindInSchema(s.Parsed, keyword, scope)
}

// FindInSchema searches a parsed ast.Schema for definitions matching keyword and scope.
func FindInSchema(schema *ast.Schema, keyword string, scope FindScope) ([]FindResult, error) {
